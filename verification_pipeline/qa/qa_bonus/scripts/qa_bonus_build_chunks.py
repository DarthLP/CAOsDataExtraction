"""qa_bonus_build_chunks.py — Stage 3 setup for bonus.

Per Phase 0.5, bonus shares the token-heavy wage_information.md, so per-item
...
def main():
    print("=== Stage 3 setup (bonus) — raised caps 3500/6000 ===")
    ...
    prompt = worksheet_builder.build_subagent_prompt("bonus")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    print(f"  system prompt: {len(prompt)//4} tokens")

    logging_util.log_event("bonus", "stage_3_setup", "chunks built",
