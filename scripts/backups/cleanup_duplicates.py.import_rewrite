...
def cleanup_duplicates():
    ...
    backup_file = f"performance_logs/extraction_performance_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
    ...
    # Remove duplicates (keep the latest entry for each filename+success combination)
    unique_entries = {}
    ...
    for entry in entries:
        ...
        # Create unique key (same file in same CAO = duplicate, regardless of API key or success)
        # Keep the latest attempt for each file
