#!/usr/bin/env python3
"""
Complete CAO data translation script using Gemini 2.5 Pro
Translates both column headers and data values with proper rate limit handling
Usage: python translate_cao_complete.py [sheet_name]
"""
...
def query_gemini_with_retry(prompt, max_retries=3):
    """
    Query Gemini 2.5 Pro with proper retry logic and rate limit handling.
    """
    for attempt in range(max_retries):
        try:
            ...
            if hasattr(response, 'candidates') and response.candidates:
                ...
                if hasattr(candidate, 'finish_reason'):
                    if candidate.finish_reason == 1:  # SAFETY
                        print(f"    Content blocked by safety filters, trying simpler prompt...")
                        # Try with a much simpler prompt
                        simple_prompt = f"Translate to English: {prompt.split('Translate')[1] if 'Translate' in prompt else prompt}"
                        response = model.generate_content(simple_prompt, generation_config=generation_config)
                    elif candidate.finish_reason in [2, 3]:  # RECITATION or OTHER
                        print(f"    Finish reason {candidate.finish_reason}, retrying...")
            ...
            if "429" in error_str or "quota" in error_str:
                wait_time = 120 * (2 ** attempt)  # 2min, 4min, 8min
...
def translate_column_headers_complete(columns, sheet_name, sample_data):
    """
    Translate all column headers using Gemini 2.5 Pro.
    """
    print(f"  Translating {len(columns)} column headers...")
    
    # Prepare the prompt with context
    columns_str = ", ".join(columns)
    sample_str = json.dumps(sample_data, indent=2, default=str)[:2000]  # Limit sample size
...
Only return valid JSON, no markdown or explanations."""
    ...
    if result:
        ...
        print(f"    No response from Gemini for column headers")
        return {"translations": {}, "abbreviations": {}}

def translate_data_values_complete(df, translations, sheet_name):
    """
    Translate data values using Gemini 2.5 Pro.
    """
    print(f"  Translating data values...")
    
    translated_df = df.copy()
    
    # Identify text columns (not codes, dates, numbers)
    text_columns = []
    for col in df.columns:
        if col in translations.get("translations", {}):
            ...
            if len(sample_values) > 0:
                # If more than 30% of values contain letters
                text_ratio = sum(1 for val in sample_values if re.search(r'[a-zA-Z]', val)) / len(sample_values)
                if text_ratio > 0.3:
                    text_columns.append(col)
    
    print(f"    Found {len(text_columns)} text columns to translate")
    
    # Process text columns in small batches
    ...
    for i in range(0, len(text_columns), batch_size):
        ...
        for col in batch_cols:
            ...
            # Limit to most common values to avoid token limits
            if len(unique_values) > 15:
                # Get value counts and take most common
...
def translate_sheet_complete(sheet_name):
    """
    Complete translation of a sheet including headers and data values.
    """
    print(f"\n=== Complete Translation: {sheet_name} ===")
    ...
    try:
        ...
        translation_result = translate_column_headers_complete(list(df.columns), sheet_name, sample_data)
        ...
        translated_df = translate_data_values_complete(translated_df, translation_result, sheet_name)
        
        # Step 4: Save translated sheet
        output_path = f"_EXTRA/{sheet_name.replace(' ', '_').replace('-', '_')}_complete_translated.xlsx"
        translated_df.to_excel(output_path, index=False)
        print(f"Saved complete translation to: {output_path}")
        
        # Step 5: Update translation helper
        update_helper_complete(translation_result, sheet_name)
...
def update_helper_complete(translation_result, sheet_name):
    """
    Update translation helper with complete translation results.
    """
    try:
        ...
        for abbrev, meaning in translation_result.get("abbreviations", {}).items():
            new_entries.append({
                'Dutch': abbrev,
                'Translation': meaning,
                'Explanation': f'Abbreviation from {sheet_name}',
...
def main():
    ...
    print(f"Complete CAO Data Translation with Gemini 2.5 Pro")
    print(f"Sheets to process: {sheets_to_process}")
    print(f"Note: This will translate BOTH column headers AND data values")
    print(f"Note: Rate limits are handled with proper waiting periods")
    ...
    for i, sheet_name in enumerate(sheets_to_process):
        ...
        success = translate_sheet_complete(sheet_name)
        if not success:
    ...
    print(f"\n{'='*60}")
    print("Complete translation process finished!")
    print(f"{'='*60}")
