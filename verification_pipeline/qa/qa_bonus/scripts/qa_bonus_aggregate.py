"""qa_bonus_aggregate.py — Stage 4 aggregator + audit for bonus.

Full pipeline with all data-quality guards wired in (in order):
  csv_recovery → aggregate → suppress boolean-on-numeric → audit (incl. A18)
  → flag NHR → suppress unit-without-value.
"""
...
def main():
    print("=== Stage 4 (bonus) — aggregate + audit ===")
    ...
    logging_util.log_event("bonus", "stage_4", "aggregate+audit",
