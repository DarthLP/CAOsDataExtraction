"""qa_leave_build_chunks.py — Stage 3 setup for leave (standardized re-run)."""
...
def main():
    print("=== Stage 3 setup (leave, standardized re-run) — chunk build ===")
    ...
    chunks = worksheet_builder.build_chunk_items(records, "leave")
    ...
    prompt = worksheet_builder.build_subagent_prompt("leave")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    logging_util.log_event("leave", "stage_3_setup", "chunks built (standardized re-run)",
