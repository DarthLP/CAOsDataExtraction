"""qa_term_build_chunks.py — Stage 3 setup: build chunk JSONLs + system prompt (term)."""
...
def main():
    print("=== Stage 3 setup (term) — chunk build ===")
    ...
    chunks = worksheet_builder.build_chunk_items(records, "term")
    ...
    prompt = worksheet_builder.build_subagent_prompt("term")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    tokens = len(prompt) // 4
    print(f"  system prompt: {tokens} tokens")

    logging_util.log_event("term", "stage_3_setup", "chunks built",
                           chunks=len(chunks),
