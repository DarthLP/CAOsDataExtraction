"""qa_pension_aggregate.py — Stage 4 aggregator + audit + era-outlier flagging (pension).
...
suppress unit-without-value -> POST-HOC era-baseline outlier flag (accrual cap /
franchise floor; surface only, never auto-changes data).
"""
...
def main():
    print("=== Stage 4 (pension) — aggregate + audit + era flag ===")
    rec = csv_recovery.recover_glob(QA / "outputs" / "subagent_worksheets" / "chunks")
    print(f"  csv recovery: {rec}")
    ...
    outliers = era_baselines.flag_topic_outliers(
        "pension", scoped_df.to_dict("records"), corr_df.to_dict("records"))
    ...
    logging_util.log_event("pension", "stage_4", "aggregate+audit+era",
