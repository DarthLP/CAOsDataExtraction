"""qa_fringe_build_chunks.py — Stage 3 setup for fringe (default caps)."""
...
def main():
    ...
    chunks = worksheet_builder.build_chunk_items(records, "fringe")
    ...
    prompt = worksheet_builder.build_subagent_prompt("fringe")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    print(f"  system prompt: {len(prompt)//4} tokens")
    logging_util.log_event("fringe", "stage_3_setup", "chunks built",
