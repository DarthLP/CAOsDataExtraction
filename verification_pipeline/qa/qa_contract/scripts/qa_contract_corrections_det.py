"""qa_contract_corrections_det.py — Stage 2 driver for contract."""
...
import qa_contract_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_contract_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    presence_df = presence_scan.scan_topic_presence(
        "contract", scoped, field_specific=True)
    ...
    # Enum-mismatch scan (none for contract but keep scaffold)
    enum_fields = schema_lookup.get_topic_enum_fields("contract")
    enum_rows = []
    ...
    logging_util.log_event("contract", "stage_2",
