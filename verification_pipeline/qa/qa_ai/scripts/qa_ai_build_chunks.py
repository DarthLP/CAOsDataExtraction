"""qa_ai_build_chunks.py — Stage 3 setup for ai (default caps)."""
...
def main():
    ...
    chunks = worksheet_builder.build_chunk_items(records, "ai")
    ...
    prompt = worksheet_builder.build_subagent_prompt("ai")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    print(f"  system prompt: {len(prompt)//4} tokens")
    logging_util.log_event("ai", "stage_3_setup", "chunks built",
