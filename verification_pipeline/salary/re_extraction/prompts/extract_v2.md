# Salary extraction prompt — v2 (patched)

Adds **[EDIT-5]** on top of v1 to handle the b0032-style regression case:
text describes scale STRUCTURE but does not tabulate amounts (e.g. "see
Appendix 2", "to be agreed"). Also references a per-CAO overrides file
(`cao_overrides.md`) for irreducible quirks (loaded conditionally).
...
- **[EDIT-5]** STRUCTURE-ONLY EXCEPTION TO [EDIT-4]. If — and ONLY IF — the wage_information text:
  - (a) explicitly names jobgroups/scales/function groups by label (e.g. "Functiegroepen 1-9 / 0-H", "Schaal A-J"),
  - (b) provides effective dates AND/OR general %-increases for that wage system,
  - (c) but does NOT print a single euro/hourly/monthly amount for any of those groups anywhere in the text (typical phrases: "see Appendix X", "amounts available at <URL>", "to be agreed", "nntb", "nader te bepalen", "WML" alone with no concrete value),
  then DO emit one SalaryRow per identifiable jobgroup with `timeline: []` and `row_note` set to a single sentence stating where the amounts are (verbatim from text: "Salary amounts not tabulated in this section; see Appendix N" or similar). This is the ONLY case where an empty timeline is allowed. Do NOT use this as an escape hatch when partial amounts ARE present elsewhere in the text — the rule fires only when the WHOLE wage table is non-tabulated.
