"""qa_pension_scope.py — Stage 1 scope filter for pension topic (source: pension_information.md)."""
...
def main():
    print("=== Stage 1 (pension) ===")
    full = scope_filter.load_full_csv()
    scoped = scope_filter.most_recent_doc_per_cao(full, "pension")
    print(f"  scoped: {len(scoped)} ({scoped['cao_number'].nunique()} unique CAOs)")
    OUT.parent.mkdir(parents=True, exist_ok=True)
    scoped.to_csv(OUT, sep=";", index=False)
    logging_util.log_event("pension", "stage_1", "scope filter complete",
