"""qa_training_corrections_det.py — Stage 2 driver for training.

Runs L1 rules + L2 field-specific presence scan + ENUM-mismatch scan,
combines into corrections_deterministic.csv.
"""
...
import qa_training_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_training_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    # 3. Enum-value mismatch scan (training schema has no enums, but keep
    # the scaffolding so future topics use the same pattern)
    enum_fields = schema_lookup.get_topic_enum_fields("training")
    enum_rows = []
    ...
    logging_util.log_event("training", "stage_2",
