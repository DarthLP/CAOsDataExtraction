...
def _ken_sortkey(m):
    """DD/MM/YYYY -> (0, YYYY, MM, DD) so earliest sorts first; missing -> (1,...) sorts last.
    id is the stable tie-break (NOT a recency signal, just determinism)."""
    k = (m.get('datum_kennisgeving') or '').strip()
    idv = str(m.get('id') or '')
    idnum = int(idv) if idv.isdigit() else 10**9
    if len(k) == 10 and k[2] == '/' and k[5] == '/':
        return (0, k[6:10], k[3:5], k[0:2], idnum)
    return (1, '', '', '', idnum)


def load_document_type(path=NON_SALARY_CSV):
    """file_name -> general_document_type (deltas vs full CAO). Empty map if the CSV is absent."""
    dt = {}
    if not os.path.exists(path):
        return dt
    for r in csv.DictReader(open(path), delimiter=';'):
        fn = r.get('file_name')
        if fn and fn not in dt:
            dt[fn] = r.get('general_document_type', '') or ''
    return dt


def build_version_map(meta_by_file, doc_type=None):
    """meta_by_file: file_name -> {cao_number, ingangsdatum, datum_kennisgeving, id}.
    Returns file_name -> {term_group, kennisgeving_rank, base_id, n_editions, document_type}."""
    if doc_type is None:
        doc_type = load_document_type()
    groups = collections.defaultdict(list)
    for fn, m in meta_by_file.items():
        cao = str(m.get('cao_number') or '')
        ing = (m.get('ingangsdatum') or '').strip()
        # empty ingangsdatum -> its own singleton group (never falsely merge unrelated editions)
        tg = f"{cao}|{ing}" if ing else f"{cao}|file:{fn}"
        groups[tg].append((fn, m))
    out = {}
    for tg, files in groups.items():
        ordered = sorted(files, key=lambda it: _ken_sortkey(it[1]))
        base_id = str(ordered[0][1].get('id') or '')
        n = len(ordered)
        for rank, (fn, m) in enumerate(ordered, 1):
            out[fn] = {'term_group': tg, 'kennisgeving_rank': rank, 'base_id': base_id,
                       'n_editions': n, 'document_type': doc_type.get(fn, '')}
    return out
