"""qa_wage_corrections_det.py — Stage 2 driver for wage (source: wage_information.md)."""
...
import qa_wage_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_wage_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    presence_df = presence_scan.scan_topic_presence("wage", scoped, field_specific=True)
    ...
    enum_fields = schema_lookup.get_topic_enum_fields("wage")
    enum_rows = []
    for f, valid in enum_fields.items():
        ...
        for _, r in scoped.iterrows():
            ...
            enum_rows.append({
                ...
                "topic_group": "wage", "confidence": "", "evidence_quote": "",
    ...
    logging_util.log_event("wage", "stage_2", "deterministic corrections produced",
