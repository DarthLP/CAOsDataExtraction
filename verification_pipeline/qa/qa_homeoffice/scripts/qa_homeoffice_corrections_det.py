"""qa_homeoffice_corrections_det.py — Stage 2 driver for homeoffice.

Runs L1 rules + L2 presence scan (with field-specific filter), combines
into corrections_deterministic.csv. Also flags enum-value mismatches on
populated `discretion` fields.
"""
...
import qa_homeoffice_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_homeoffice_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    # 2. L2 presence scan with field-specific filtering
    source_text_loader.clear_cache()
    presence_df = presence_scan.scan_topic_presence(
        "homeoffice", scoped, field_specific=True)
    print(f"  L2 presence triggers (field-specific): {len(presence_df)}")

    # 2a. Drop L2 rows whose (rid, field) already flagged by L1
    ...
    # 3. Enum-value mismatch scan
    enum_fields = schema_lookup.get_topic_enum_fields("homeoffice")
    enum_rows = []
    for f, valid in enum_fields.items():
        ...
        for _, r in scoped.iterrows():
            ...
            if not v or v.lower() in ("nan", "unknown"):
                continue
            # Skip if already in valid set (case-sensitive match required)
            if v in valid:
                continue
            # Also skip if it's the L1 R5 catch (already flagged)
    ...
    if len(out_df):
        ...
        for rid, n in out_df.groupby("rule_id").size().sort_values(ascending=False).items():
            print(f"  {rid}: {n}")
        print("\nBy worksheet_mode:")
        for wm, n in out_df.groupby("worksheet_mode").size().sort_values(ascending=False).items():
            print(f"  {wm}: {n}")

    logging_util.log_event("homeoffice", "stage_2",
                           "deterministic corrections produced",
                           rule_violations=len(rule_rows),
                           l2_triggers=len(presence_rows),
                           enum_mismatches=len(enum_rows),
                           total=len(out_df))
