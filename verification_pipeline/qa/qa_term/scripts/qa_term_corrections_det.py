"""qa_term_corrections_det.py — Stage 2 driver for term (L1 rules + L2 presence + ENUM)."""
...
import qa_term_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_term_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    presence_df = presence_scan.scan_topic_presence("term", scoped, field_specific=True)
    ...
    enum_fields = schema_lookup.get_topic_enum_fields("term")
    enum_rows = []
    ...
    logging_util.log_event("term", "stage_2", "deterministic corrections produced",
