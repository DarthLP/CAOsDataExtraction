...
class CAOTracker:
    def __init__(self, excel_path="inputs/excel/inputExcel/CAO_Frequencies_2014.xlsx"):
    ...
    def count_pdfs_in_folder(self, cao_number):
        ...
        pdf_folder = Path(f"inputs/pdfs/input_pdfs/{cao_number}")
    ...
    def get_failed_pdf_files(self, cao_number):
        ...
        pdf_folder = Path(f"inputs/pdfs/input_pdfs/{cao_number}")
        output_folder = Path("outputs/parsed_pdfs/parsed_pdfs_json") / str(cao_number)
    ...
    def get_failed_llm_files(self, cao_number):
        ...
        input_folder = Path("outputs/parsed_pdfs/parsed_pdfs_json") / str(cao_number)
    ...
    def get_failed_llm_analysis_files(self, cao_number):
        ...
        input_folder = Path("outputs/llm_extracted") / str(cao_number)
        results_excel = Path("outputs/excel/extracted_data.xlsx")
    ...
    def auto_update_from_files(self):
        ...
        for _, row in self.df.iterrows():
            ...
            # Count extracted JSONs (outputs/parsed_pdfs/parsed_pdfs_json folder)
            extracted_count = self.count_json_files("outputs/parsed_pdfs/parsed_pdfs_json", cao_number)
