"""qa_childcare_corrections_det.py — Stage 2 driver for childcare."""
...
import qa_childcare_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_childcare_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    presence_df = presence_scan.scan_topic_presence("childcare", scoped, field_specific=True)
    ...
    for _, p in presence_df.iterrows():
        presence_rows.append({
            ...
            "topic_group": "childcare", "confidence": "",
    ...
    enum_fields = schema_lookup.get_topic_enum_fields("childcare")
    enum_rows = []
    for f, valid in enum_fields.items():
        ...
        for _, r in scoped.iterrows():
            ...
            enum_rows.append({
                ...
                "topic_group": "childcare", "confidence": "", "evidence_quote": "",
    ...
    if len(out_df):
        for rid, n in out_df.groupby("rule_id").size().sort_values(ascending=False).items():
            print(f"  {rid}: {n}")
    logging_util.log_event("childcare", "stage_2", "deterministic corrections produced",
