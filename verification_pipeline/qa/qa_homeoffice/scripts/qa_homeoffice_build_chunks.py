"""qa_homeoffice_build_chunks.py — Stage 3 setup: build chunk JSONLs + system prompt (homeoffice).

Added 2026-05-27 for template consistency (homeoffice originally built chunks ad-hoc).
...
def main():
    print("=== Stage 3 setup (homeoffice) — chunk build ===")
    ...
    chunks = worksheet_builder.build_chunk_items(records, "homeoffice")
    ...
    prompt = worksheet_builder.build_subagent_prompt("homeoffice")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    logging_util.log_event("homeoffice", "stage_3_setup", "chunks built",
