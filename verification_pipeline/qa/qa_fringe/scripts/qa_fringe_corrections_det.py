"""qa_fringe_corrections_det.py — Stage 2 driver for fringe."""
...
import qa_fringe_rules  # noqa: E402
...
def main():
    ...
    for _, row in scoped.iterrows():
        ...
        for rule in qa_fringe_rules.RULES:
            v = rule(row, rid)
            if v is not None:
    ...
    presence_df = presence_scan.scan_topic_presence("fringe", scoped, field_specific=True)
    ...
    enum_fields = schema_lookup.get_topic_enum_fields("fringe")
    enum_rows = []
    for f, valid in enum_fields.items():
        ...
        for _, r in scoped.iterrows():
            ...
            enum_rows.append({
                ...
                "topic_group": "fringe", "confidence": "", "evidence_quote": "",
    ...
    if len(out_df):
        for rid, n in out_df.groupby("rule_id").size().sort_values(ascending=False).items():
            print(f"  {rid}: {n}")
    logging_util.log_event("fringe", "stage_2", "deterministic corrections produced",
