"""qa_homeoffice_aggregate.py — Stage 4 aggregator + audit for homeoffice.

Reproducible driver (the original homeoffice run was done inline). Runs
csv_recovery → aggregate → suppress boolean-on-numeric → audit (incl. A18)
→ flag NHR → suppress unit-without-value. Mirrors the other topic drivers.
"""
...
def main():
    print("=== Stage 4 (homeoffice) — aggregate + audit ===")
    ...
    logging_util.log_event("homeoffice", "stage_4",
                           "aggregate+audit re-run (recovery+A18+suppressors)",
