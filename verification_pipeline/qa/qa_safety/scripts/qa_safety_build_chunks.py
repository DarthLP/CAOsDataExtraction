"""qa_safety_build_chunks.py — Stage 3 setup for safety (default caps)."""
...
def main():
    ...
    chunks = worksheet_builder.build_chunk_items(records, "safety")
    ...
    prompt = worksheet_builder.build_subagent_prompt("safety")
    (WS / "system_prompt.txt").write_text(prompt, encoding="utf-8")
    print(f"  system prompt: {len(prompt)//4} tokens")
    logging_util.log_event("safety", "stage_3_setup", "chunks built",
