"""qa_wage_scope.py — Stage 1 scope filter for wage topic (source: wage_information.md)."""
...
def main():
    ...
    scoped = scope_filter.most_recent_doc_per_cao(full, "wage")
    print(f"  scoped: {len(scoped)} ({scoped['cao_number'].nunique()} unique CAOs)")
    OUT.parent.mkdir(parents=True, exist_ok=True)
    scoped.to_csv(OUT, sep=";", index=False)
    logging_util.log_event("wage", "stage_1", "scope filter complete",
