...
- **Auxiliary export**: `scripts/export_top100_leave_timeline.py` reads p3 `outputs/llm_extracted/new_flow/[CAO_NUMBER]/*_extract.json` plus `extracted_cao_info*.csv` metadata and writes leave timeline CSV/JSONL outputs to `outputs/llm_extracted/excel_test/` (including `per_cao/{cao}_Leave_Timeline.jsonl`).
- **Auxiliary export**: `scripts/export_p3_all_sections_markdown.py` reads the same p3/metadata sources and writes markdown bundles under `outputs/llm_extracted/excel_test/all_sections/` (`by_topic/*.md`, `ALL.md`, `ALL_EXCEPT_SALARY.md`).
- **Auxiliary export**: `scripts/export_prompt_schema_markdown.py` writes prompt/schema reference markdown to `outputs/llm_extracted/excel_test/` (`NON_SALARY_PROMPTS_AND_SCHEMA.md`, `SALARY_PROMPTS_AND_SCHEMA.md`). The non-salary reference is intentionally compact: one general prompt section, then per-topic p4 summary descriptions + schema description + field/type blocks (no repeated full prompt text per topic).
...
- **Salary latest + increase-by-salary-year latest:** `latest_cao_active_file_table` (nominal forward-fill) is passed through `**snap_active_table_to_band_eligible_salary_files`** in `salary_plot_cohort_utils.py`, using governed band-eligible long rows from `**build_governed_band_eligible_slot_long`** (`descriptives_salary_plots.py`). Per CAO and calendar year, if the nominal active file has no such rows, the **previous** file that did is carried until the new file qualifies—so plots do not switch to a contract PDF with zero band-eligible salary slots. **Boolean shares** (`salary_boolean_shares_by_contract_year*.png`): latest view keeps **nominal** forward-fill only (no snap); both regular and latest cohorts **drop wide rows with no strictly positive coerced `salary_k_amount`** via `**wide_row_has_any_positive_salary_amount**` in `analysis_utils.py` (aligned with long build / `n_salary_points_per_row`, not monthly band eligibility). **FT hours** (`salary_ft_hours_by_contract_year.png`) is **non-latest only** (contract cohort) on the full wide extract. `**salary_points_per_row_by_year.png`** is the same contract-cohort view but restricts to the **analytic wide sample** (`wide_row_has_any_positive_salary_amount`, aligned with `descriptives_salary.py`); the plotted statistic is **band-eligible** slot counts per row.
- Forward-fill is limited to state/context selection; event variables are not forward-filled.
- `convert_salary_to_monthly` (analysis_utils) normalizes **four-weekly** pay as `**amount × (13/12)`** (13 four-week periods per calendar year, then ÷12 months). **Daily** pay, compact `**d`**, and offshore day phrasing (`offshore day`, trailing `offshore da`, `per offshore`…) use the same `amount × 5 workdays × 4.33` rule. Hourly detection excludes `**N-hour` duration** units (e.g. `3-hour activity`) so they are not treated as €/h. All normalized monthly values are **rounded to 2 decimals**; amounts pass through `coerce_salary_amount_scalar` (EU `2.230,91` vs US `2,230.91`). Salary diagnostics/regression CSVs under `outputs/analysis/` use `sep=';'` and `**decimal=','`** to reduce Excel locale misreads of long float strings.
- Salary increase analytics are standardized with three event-level series:
  ...
  - `increase_merged_pref_csv` (CSV preferred, derived fallback; **not** masked on `analysis_monthly_band_ok` in the events frame—band-only displays and FE regressions intersect `analysis_monthly_band_ok` where the pre-unmask sample must be preserved).
- **Monthly band**: `analysis_monthly_floor_eur` stores the full NL statutory gross monthly minimum for `salary_start_date` (`conf/nl_statutory_minimum_monthly_gross_eur.csv`, 1 Jan schedule; `scripts/excel_analysis/nl_minimum_wage_monthly.py`). Band eligibility requires `amount_monthly` between `**SALARY_ANALYSIS_MONTHLY_FLOOR_RELATIVE_MIN` × that floor** (default **0.9**) and `**SALARY_ANALYSIS_MONTHLY_CAP_EUR`** in `analysis_utils.py`. Shared vectorized check: `compute_analysis_monthly_floor_and_band_ok` in `salary_increase_derivation.py` (also used for band-aligned salary **level** plots). Diagnostics append band failures; `derive_salary_increase_series` returns `band_summary`. `descriptives_salary.py` logs counts, writes `outputs/analysis/salary_monthly_band_summary.csv`, and adds section `salary_monthly_band` to `**02_sample_overview`**. `**descriptives_salary_plots.py`** refreshes the same `salary_monthly_band_summary.csv` and prints the band line when plots are run without the full descriptives workbook. Band summary shares use `**n_conversion_ok**` as denominator. Salary level trend PNGs: `salary_amount_monthly_eur_band_eligible_by_salary_year.png` (salary start year cohort, newest-file overlap) and `salary_amount_monthly_eur_band_eligible_by_contract_year.png` (contract cohort from `ingangsdatum`, governing file, all band-eligible slots on that file) use the same normalization + band as increases (boxplot fliers hidden). `**salary_amount_monthly_eur_band_eligible_by_contract_year_latest_cao_view.png**` keeps the legacy filename but puts **calendar year** on the x-axis and includes **all** band-eligible long slots on the **snapped** active file per `(CAO, T)` (`snap_active_table_to_band_eligible_salary_files` after `latest_cao_active_file_table`, then `build_contract_year_latest_salary_calendar_panel`). `**salary_amount_monthly_eur_band_eligible_by_salary_year_latest_cao_view.png`** uses `**build_salary_year_latest_gap_panel`**: per-slot latest step with `salary_start_date` ≤ end(`T`) on that **snapped** file, plus **file-transition gap carry** when a new file has no such rows yet. `**outputs/analysis/salary_plot_years_dropped.csv`** is a **header-only** placeholder (salary descriptive plots do not drop cohort years by minimum *n*). **Other cohort helpers** in `scripts/excel_analysis/salary_plot_cohort_utils.py`: governing file keys, overlap on salary start year, `**latest_cao_active_file_table`**, `**snap_active_table_to_band_eligible_salary_files`**, latest increase panel with **0%** when no event in `T`. Spaghetti plot: black line = **CAO-equal weighted** mean of merged increase on the overlap-resolved panel; **twin axis** = cohort-local CAO counts for that grand-line sample. Comparison and shift plots use weighted yearly means with CAO twins on the merged-series / merged-shift sample.
- **Salary descriptives analytic sample**: `descriptives_salary.py` restricts wide descriptives sheets **01–08** to rows with `**n_salary_points_per_row` > 0** (same rule as long build / `wide_row_has_any_positive_salary_amount`). The filtered frame carries `**_wide_source_row_id`**; `wide_analysis_row_ids` in `analysis_utils.py` supplies original CSV `**row_id`** indices for `build_long_salary_from_wide` and `derive_salary_increase_series`. `**outputs/analysis/salary_descriptives_excluded_no_salary.csv**` plus workbook sheet `**00_excluded_no_salary_summary**` summarize excluded wide rows and files where **every** wide row for that `(cao_number, file_name)` has zero salary points. Sheet **09** (`09_no_salary_analysis`) still uses the full extract.
- **Descriptive increase plots** (`descriptives_salary_plots.py`): **Contract start year** merged-increase aggregates are saved as `salary_increase_percent_by_contract_year*.png` (legacy misleading names `salary_increase_percent_by_salary_year*.png` are obsolete and should not be regenerated). Regular path uses governing-file–restricted events and CAO-equal weights; **latest** path forward-fills each CAO’s contract-mean merged increase across calendar years. **Salary year** latest-increase views (`*_by_salary_year_latest_cao_view.png`) use the **snapped** active file per `(CAO, T)` (same as latest salary level) and impute **0%** when there is no band-eligible non-null event in `T` on that file for that series (stock vs flow: no carry-forward of last increase). Regression/event-level work still uses observed events as defined in the regression module.
- **Salary descriptive verification (QA):** weighted cohort mean ≡ mean of per-CAO means on the figure’s final `S_y`; weighted box stats reproducible from the same frame + `cao_weight`; Latest salary level: no duplicate `(calendar_year or salary_year, file_name, row_id, salary_index)` per `(CAO, T)` after dedup; Latest increase: imputed-0 CAOs included in twin CAO counts; normal contract cohort: governing file only; Latest contract salary: **snapped** active file, all band-eligible slots; shift: yearly point = CAO-equal weighted mean of shift rows. `validate_analysis_outputs.py` requires the full salary PNG set listed in its `REQUIRED_FILES` (aligned with the module docstring OUTPUT list in `descriptives_salary_plots.py`).
- Derived increase QA artifacts are persisted in `outputs/analysis/` as semicolon-separated CSVs: `salary_increase_events_derived.csv`, `salary_increase_conversion_diagnostics.csv` (always emitted by salary descriptives), and `salary_increase_csv_vs_diff_comparison.csv` (CSV vs diff comparison with `abs_diff_gt_0_1` among other columns). `**salary_band_and_conversion_diagnostics.csv`** (from `**descriptives_salary_plots.py`**) combines wide-slot exclusions, long-row conversion/band failures, per-CAO summaries when no band-eligible slot exists, and global reason counts (`record_type` column).
...
- Regression outputs: `scripts/excel_analysis/salary_increase_regression.py` writes `outputs/analysis/salary_regression_event_level.csv`, `salary_regression_transition_level.csv`, and `**salary_regression_fit_metrics.csv`** (per-model `**r2`**, `**r2_within**`, `**adj_r2**`, `**adj_r2_within**`, `**rmse**`, formula, ref year, n_obs, clusters). Event-level and transition samples for `**increase_merged_pref_csv**` intersect `**analysis_monthly_band_ok**` so coefficients stay on the band-eligible subset after merged values were unmasked in events. Plotting lives in `**scripts/excel_analysis/salary_regression_plotting.py**`; figures go to `**outputs/analysis/figures/salary_regression/**` (event: two-panel coef plot + interactions-only companion per outcome; transition: year coefficients for `delta_file_mean_increase`). **Shaded bands** = approximate **95% CI** (±1.96×SE) from **CRV1** vcov; legends label the fill `**95% CI`**; panel B uses SE for Var(β_NF + β_NF×y). Bottom captions reserve margin so axis labels do not overlap notes. `**add_yearly_variance_layer`** in `descriptives_salary_plots.py`: `**hide_boxplot_fliers=True**` sets `**showfliers=False**`; otherwise fliers are shown. Box statistics use **weighted** quartiles + `**bxp`** (see `salary_plot_cohort_utils.py`). Salary increase axis conventions: single-series derived/merged/csv plots (salary start year and latest salary year) and `salary_increase_percent_by_contract_year*` use **[-4, 12]**; `salary_increase_series_comparison_by_year` uses **[0, 6]**. `salary_increase_percent_by_contract_year*` uses **Contract start year** on the x-axis; latest-increase-by-salary-year files use **Salary year**. Coefficient tables use `**Coefficient`**, `**formula`**, `**ref_year**`, `**se_invalid**`, standard inference columns, `**outcome**` (event only), sample/cluster counts; sep `;`, `decimal=','`. Reference year is the minimum factor level kept after sparse-year drops. Wide salary CSV load uses header-derived `**usecols**`; scoped `**DtypeWarning**` suppression on the subset read when needed.
...
6. Optional: run `conda run -n caos-extract python scripts/export_top100_leave_timeline.py` to export merged/per-CAO leave timelines from p3 outputs
7. Optional: run `conda run -n caos-extract python scripts/export_p3_all_sections_markdown.py` to export p3 section markdown files (`by_topic`, `ALL`, `ALL_EXCEPT_SALARY`)
